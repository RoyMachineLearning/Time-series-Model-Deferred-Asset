...
#read the csv file - Actuals for Cash from 201501 to 201710 - Update this every month 
dataSeries = read_csv('TS_Deffered.csv', parse_dates=['Time'], index_col='Time',date_parser=dateparse)
...
ts = dataSeries['DEFD_TAX_ASSETS'].astype(np.float64) 
...
model = ARIMA(ts, order = (1,1,0)) #high correlations until level 2
...
Training_Size = int(len(Predicted_data_Series) * .67)
...
for i in range(len(test)):
    ...
    model = ARIMA(Actuals, order = (1,1,0)) #high correlations until level 2
    model_fit = model.fit(disp = 0)
